...
void resolver(int N, MapTree<string, vector<int>> &map) {
  string line;
  string word, wordM;
  for(int i = 1; i <= N; i++){
    getline(cin, line);
    ...
    while(iss >> word){
      ...
      if(wordM.size() > 2){
        if(map[wordM].size() == 0 || map[wordM].back() != i)
          map[wordM].push_back(i);
      }
    }
  }
}

void print_sol(MapTree<string, vector<int>> &sol){
  MapTree<string, vector<int>>::const_iterator it = sol.cbegin();

  while(it != sol.cend()){
    cout << it->key;
    
    for(long unsigned int i = 0; i < it->value.size(); i++){
      cout << " " << it->value[i];
    }
    cout << endl;
    ++it;
