...
/*@ <answer> */
  ...
  int despachoJefeLibre(int ultimo, MapTree<int, int> despachos){
    while(despachos.contains(ultimo)) ultimo++;
    return ultimo;
  }

  int despachoEmpleadoLibre(int ultimo, MapTree<int, int> despachos, int max){
    while(despachos.contains(ultimo) && despachos[ultimo] >= max) ultimo++;
    return ultimo;
  }
 
 MapTree<int, int> organizarDespachos(int max, stack<int> jefes, stack<int> empleados){
  MapTree<int, int> despachos;
  while(!jefes.empty()){
    if(!despachos.contains(jefes.top())) despachos.insert({jefes.top(), max + 1});
    else despachos.insert({despachoJefeLibre(jefes.top(), despachos), max + 1});
    jefes.pop();
  }

  while(!empleados.empty()){
    if(!despachos.contains(empleados.top())) despachos.insert({empleados.top(), 1});
    else despachos[despachoEmpleadoLibre(empleados.top(), despachos, max)]++;
    empleados.pop();
  }
...
bool tratar_caso() {
  ...
  stack<int> jefes, empleados;

  int despacho;
  while(N--){
   cin >> despacho;
   empleados.push(despacho);
  }
  ...
  MapTree<int, int> despachos = organizarDespachos(C, jefes, empleados);

  for (auto elem : despachos) {
    cout << elem.key << " ";
    if (elem.value == C + 1) cout << "JEFE";
