...
public:
  ...
  void colarse(int pringado, QueueLinkedList &colegas){
    Node *cur = front_node;
    while(cur != nullptr && cur->value != pringado) cur = cur->next;

    if(cur != nullptr){
      Node *sig = cur->next;
      if(!colegas.empty()){
        Node *front = colegas.front_node, *back = colegas.back_node;
        cur->next = front;
        back->next = sig;
      }
    }
