...
public:
  ...
  ListLinkedSingle<Elem> mix(ListLinkedSingle<Elem> &l2);
...
template<typename Elem>
void ListLinkedSingle<Elem>::duplicate(){
  if(head->next != nullptr){
    Node *current = head->next;
  
    while (current != nullptr){
      Node *copy = new Node {current->value, current->next};
      current->next = copy;
      current = copy->next;
    }
  }
...
template<typename Elem>
void ListLinkedSingle<Elem>::reverse(){
  if(head->next != nullptr){
    Node *current = head->next;
    Node *aux = nullptr;
    while(current != last){
...
template<class Elem> 
ListLinkedSingle<Elem> ListLinkedSingle<Elem>::mix(ListLinkedSingle<Elem> &l2){
  if(!l2.empty()){
    Node *current1 = this->head->next;
    Node *current2 = l2.head->next;
    ListLinkedSingle<Elem> lista;
    Node *position = lista.head;

    while(current1 != nullptr || current2 != nullptr){
     if(((current1 != nullptr && current2 != nullptr) && (current1->value < current2->value)) || (current1 != nullptr && current2 == nullptr)){
        position->next = current1;
        current1 = current1->next;
        this->head->next = current1;
      }else{
        position->next = current2;
        current2 = current2->next;
        l2.head->next = current2;
      }
